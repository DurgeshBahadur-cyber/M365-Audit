function ConvertTo-MtM365AdvisorResult {
    ...
    param(
        ...
        # Optional Invoke-M365Advisor command that was run
        [Parameter(Mandatory = $false)]
        [string] $InvokeM365AdvisorCommand,
    ...
    function GetM365AdvisorLatestVersion() {
    ...
    $latestVersion = GetM365AdvisorLatestVersion
    ...
    foreach ($test in $sortedTests) {
        ...
        if ($start -gt 0) {
            $helpUrl = $name.Substring($start + 4).Trim() #Strip away the "See https://m365advisor.dev" part
            $name = $name.Substring(0, $start).Trim() #Strip away the "See https://m365advisor.dev" part
        }
    ...
    $mtTestResults = [PSCustomObject][ordered]@{
        ...
        InvokeCommand     = $InvokeM365AdvisorCommand
        MgContext         = GetMgContextInfo
        PesterConfig      = GetPesterConfigInfo $PesterConfiguration
        M365AdvisorConfig     = $__MtSession.M365AdvisorConfig
        Tests             = $mtTests
        Blocks            = $mtBlocks
