function Invoke-M365Advisor {
    ...
    .DESCRIPTION
    Using Invoke-M365Advisor is the easiest way to run the Pester tests and generate a report of the results.

    For more advanced configuration, you can directly use the Pester module and the Get-MtHtmlReport function.

    By default, Invoke-M365Advisor runs all *.Tests.ps1 files in the current directory and all subdirectories recursively.
    ...
    .PARAMETER NoLogo
    Do not show the M365Advisor logo.

    .PARAMETER NonInteractive
    This will suppress the logo when M365Advisor starts, prevent the test results from being opened in the default browser, and suppress all pretty messages.
    ...
    .EXAMPLE
    Invoke-M365Advisor ./m365advisor-tests

    Runs all the tests in the folder ./tests/M365Advisor (except for those tagged as LongRunning and Preview) and generates a report of the results in the default ./test-results folder.

    .EXAMPLE
    Invoke-M365Advisor -Tag 'CA' -IncludeLongRunning
    ...
    .EXAMPLE
    Invoke-M365Advisor -Tag 'CA', 'App' -IncludePreview
    ...
    .EXAMPLE
    Invoke-M365Advisor -OutputFolder './my-test-results'

    Runs tests and generates a report of the results in the ./my-test-results folder.

    .EXAMPLE
    Invoke-M365Advisor -OutputHtmlFile './test-results/TestResults.html'

    Runs the tests and generates a report of the results in the specified file.

    .EXAMPLE
    Invoke-M365Advisor -Path ./tests/EIDSCA

    Runs tests in the EIDSCA folder.

    .EXAMPLE
    Invoke-M365Advisor -MailRecipient john@contoso.com

    Runs the tests and sends a report of the results to an email recipient.

    .EXAMPLE
    Invoke-M365Advisor -TeamId '00000000-0000-0000-0000-000000000000' -TeamChannelId '19%3A00000000000000000000000000000000%40thread.tacv2'
    ...
    .EXAMPLE
    Invoke-M365Advisor -Verbosity Normal
    ...
    $configuration.Run.Path = './tests/M365Advisor'
    $configuration.Filter.Tag = 'CA'
    $configuration.Filter.ExcludeTag = 'App'

    Invoke-M365Advisor -PesterConfiguration $configuration
    ```

    Runs Pester tests in the ./tests/M365Advisor folder that include the 'CA' tag and exclude the 'App' tag.

    .EXAMPLE
    ```powershell
    Connect-M365Advisor -Service All
    Invoke-M365Advisor -IncludeLongRunning -IncludePreview
    ```

    Connect to all tested services and run all tests, including the long-running and preview tests.

    .LINK
    https://m365advisor.dev/docs/commands/Invoke-M365Advisor
    #>
    ...
    param (
        ...
        # Passes the output of the M365Advisor tests to the console.
        [switch] $PassThru,
        ...
        # Do not show the M365Advisor logo.
        [Parameter(HelpMessage = 'Do not show the logo when starting M365Advisor.')]
        [switch] $NoLogo,

        # The root directory for configuration drift tracking.
        [Parameter(HelpMessage = 'Specify drift root directory, see https://m365advisor.dev/docs/tests/MT.1060')]
        [string] $DriftRoot
    )
    ...
    function GetPesterConfiguration($Path, $Tag, $ExcludeTag, $PesterConfiguration) {
        ...
        if ($Path) { $PesterConfiguration.Run.Path = $Path }
            ...
            if (Test-Path -Path './powershell/tests/pester.ps1') {
                # Internal dev, exclude M365Advisor's core tests
    ...
    if ( $NonInteractive.IsPresent -or $NoLogo.IsPresent ) {
        Write-Verbose "Running M365Advisor v$Version"
    } else {
...
"@
    ...
    if (-not $DisableTelemetry) {
        Write-Telemetry -EventName InvokeM365Advisor
    }
    ...
    if ($DriftRoot) {
        ...
        if (-not (Test-Path -Path $DriftRoot)) {
            ...
            Set-Item -Path Env:\M365ADVISOR_FOLDER_DRIFT -Value $DriftRoot
            Write-Verbose "🧪 Drift root directory set to: $DriftRoot"
        }
    } else {

        # Set the default drift root directory.
        # Set-Item -Path Env:\M365ADVISOR_FOLDER_DRIFT -Value $(Join-Path -Path (Get-Location) -ChildPath "drift")
    }

    $m365advisorResults = $null

    Set-MtProgressView
    Write-MtProgress -Activity 'Starting M365Advisor' -Status 'Reading M365Advisor config...' -Force
    Write-Verbose "Reading M365Advisor config from: $Path"
    # Resolve tenant ID for tenant-specific config lookup (m365advisor-config.{tenantId}.json)
    $configTenantId = $null
    if (Test-MtConnection Graph) {
        $configTenantId = (Get-MgContext).TenantId
    }
    $__MtSession.M365AdvisorConfig = Get-MtM365AdvisorConfig -Path $Path -TenantId $configTenantId

    Write-MtProgress -Activity 'Starting M365Advisor' -Status 'Discovering tests to run...' -Force
    ...
    if ($pesterResults) {
        ...
        # Build the Invoke-M365Advisor command string from bound parameters
        $invokeM365AdvisorCommand = "Invoke-M365Advisor"
        foreach ($param in $PSBoundParameters.GetEnumerator()) {
            $paramName = $param.Key
            $paramValue = $param.Value
            if ($paramValue -is [switch]) {
                if ($paramValue.IsPresent) {
                    $invokeM365AdvisorCommand += " -$paramName"
                }
            } elseif ($paramValue -is [array]) {
                $invokeM365AdvisorCommand += " -$paramName @('$($paramValue -join "', '")')"
            } elseif ($paramValue -is [string]) {
                $invokeM365AdvisorCommand += " -$paramName '$paramValue'"
            } elseif ($null -ne $paramValue) {
                $invokeM365AdvisorCommand += " -$paramName $paramValue"
            }
        }

        $m365advisorResults = ConvertTo-MtM365AdvisorResult -PesterResults $PesterResults -OutputFiles $out -InvokeM365AdvisorCommand $invokeM365AdvisorCommand -PesterConfiguration $pesterConfig
        ...
        if (![string]::IsNullOrEmpty($out.OutputMarkdownSummaryFile)) {
            ...
            $output = Get-MtMarkdownSummaryReport -M365AdvisorResults $m365advisorResults
        ...
        if (![string]::IsNullOrEmpty($out.OutputCsvFile)) {
            ...
            Convert-MtResultsToFlatObject -InputObject $m365advisorResults -CsvFilePath $out.OutputCsvFile
        }

        if (![string]::IsNullOrEmpty($out.OutputExcelFile)) {
            Write-MtProgress -Activity 'Creating Excel workbook'
            Convert-MtResultsToFlatObject -InputObject $m365advisorResults -ExcelFilePath $out.OutputExcelFile
        }
        ...
        if ($MailRecipient) {
            ...
            Send-MtMail -M365AdvisorResults $m365advisorResults -Recipient $MailRecipient -TestResultsUri $MailTestResultsUri -UserId $MailUserId
        }

        if ($TeamId -and $TeamChannelId) {
            Write-MtProgress -Activity 'Sending Teams message'
            Send-MtTeamsMessage -M365AdvisorResults $m365advisorResults -TeamId $TeamId -TeamChannelId $TeamChannelId -TestResultsUri $MailTestResultsUri
        }

        if ($TeamChannelWebhookUri) {
            Write-MtProgress -Activity 'Sending Teams message'
            Send-MtTeamsMessage -M365AdvisorResults $m365advisorResults -TeamChannelWebhookUri $TeamChannelWebhookUri -TestResultsUri $MailTestResultsUri
        }
