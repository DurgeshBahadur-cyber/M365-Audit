...
Azure DevOps tests are now available in M365Advisor!

<!-- truncate -->

## Overview

M365Advisor now includes an *optional* suite of Azure DevOps security and resource limit tests. The tests are bundled with M365Advisor and are automatically discovered when you run `Invoke-M365Advisor`. However, they are only executed if you have an active connection to an Azure DevOps organization using the community [ADOPS PowerShell](https://www.powershellgallery.com/packages/ADOPS) module. Without a connection, the tests are skipped.
...
That's it — M365Advisor detects the Azure DevOps connection and runs the tests automatically alongside any other configured tests.
...
For automated monitoring, the following pipeline runs M365Advisor tests (including the Azure DevOps tests) on a schedule and publishes the results to an Azure Web App. The pipeline connects to both Microsoft Graph (for Entra ID tests) and Azure DevOps.

**Prerequisites:**
- An **app registration** with the required [M365Advisor permissions](https://m365advisor.dev/docs/installation#configure-permissions)
- A **workload identity federation service connection** in Azure DevOps
- An **Azure Web App** to host the report (see [M365Advisor results on Azure Web App](/blog/m365advisor-with-azdo-webapp))
  ...
  - task: AzurePowerShell@5
    inputs:
      ...
      Inline: |
        ...
        # Install and run M365Advisor tests
        mkdir m365advisor-tests
        cd m365advisor-tests
        Install-M365AdvisorTests .\tests
        Invoke-M365Advisor -OutputHtmlFile "$TempOutputFolder/index.html" -Verbosity Normal

        # Publish to Azure Web App
        $FileName = "$PWD/M365AdvisorReport$date.zip"
...
- [Setting up M365Advisor in Azure DevOps Pipelines](/docs/monitoring/azure-devops)
- [M365Advisor results on Azure Web App](/blog/m365advisor-with-azdo-webapp)
- [Deploy M365Advisor Web App with Bicep](/docs/monitoring/azure-devops-web-app-bicep)
