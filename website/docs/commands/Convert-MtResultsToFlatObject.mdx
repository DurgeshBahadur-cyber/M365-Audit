...
description: "Convert M365Advisor test results to a flattened object that can be exported to CSV or Excel."
id: Convert-MtResultsToFlatObject
title: Convert-MtResultsToFlatObject
hide_title: false
hide_table_of_contents: false
custom_edit_url: https://github.com/m365advisor365/m365advisor/blob/main/powershell/public/Convert-MtResultsToFlatObject.ps1
---

## SYNOPSIS

Convert M365Advisor test results to a flattened object that can be exported to CSV or Excel.
...
The function also supports reading M365Advisor test results from a JSON file and exporting the flattened object to a CSV.
...
Convert the M365Advisor test results in C:\path\to\results.json to a flattened object that is then returned to the pipeline.
...
Convert the M365Advisor test results in C:\path\to\results.json to a flattened object, and then export that object to an Excel file (C:\path\to\results.xlsx).
...
Convert the M365Advisor test results in C:\path\to\results.json to a flattened object, and then export that object to C:\path\to\results.csv.

## PARAMETERS

### -InputObject

Use the M365Advisor test results from the pipeline or as an input object (JSON).

```yaml
Type: PSObject
Parameter Sets: XLSX, CSV, FromInputObject
Aliases: M365AdvisorResults
...
The path of the file containing the M365Advisor test results in JSON format.
...
[https://m365advisor.dev/docs/commands/Convert-MtResultsToFlatObject](https://m365advisor.dev/docs/commands/Convert-MtResultsToFlatObject)
